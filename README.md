...
## 13. 개선작업 Google Sheet 연동

개선하고 싶은 항목을 Google Sheet에 적어두고, 원할 때 Claude Code 채팅에서 "시트 확인해서 처리해줘"라고 요청하면 처리하는 방식입니다. **정해진 시간에 무인으로 자동 실행되는 구조가 아니라, 사용자가 요청할 때만 동작합니다** — 그래서 GitHub Actions나 별도 API 과금 없이, 이 채팅에 이미 포함된 사용량으로 처리됩니다.

### 13-1. Google Sheet 컬럼

`날짜 | 개선항목 | 작업대상 | 결과 | 비고` (1행은 헤더, 2행부터 데이터). "시트 확인해줘"라고 요청하면 아래 조건을 만족하는 행만 처리 대상으로 봅니다.

- 날짜 ≤ 오늘
- 작업대상 = `YES` (대소문자 무관)
- 결과가 비어 있음 (이미 `완료`/`실패`/`승인대기`가 적힌 행은 건너뜀)

### 13-2. 처리 흐름 (요청할 때마다)

1. `npm run worklist:list`로 시트에서 처리 대상 행을 읽어옴
2. 개선항목을 현재 코드 기준으로 검토
   - **DB 스키마/RLS/인덱스 변경, destructive 작업(데이터 삭제 등), 요구사항이 모호한 작업, 큰 구조 변경**이면 코드를 바로 고치지 않고 먼저 알려드립니다. 필요하면 `supabase/migrations/`에 SQL 파일만 만들어두고 실행은 하지 않습니다 — 승인 전에는 절대 스키마를 바꾸지 않습니다.
   - 일반적인 코드 수정이면 구현합니다.
3. `npm run lint` / `npm run typecheck` / `npm test` / `npm run build`를 실행해 모두 통과하는지 확인
4. 통과하면 변경 내용을 보여드리고, **커밋/푸시는 항상 진행해도 되는지 확인 후** 진행 (지금까지 이 프로젝트에서 해온 방식과 동일)
5. 처리 결과를 `npm run worklist:mark <행번호> <완료|실패|승인대기> "<비고>"`로 시트에 기록

| 결과 | 의미 |
| --- | --- |
| `완료` | 구현하고 lint/typecheck/test/build 통과, `main`에 반영함 |
| `실패` | 구현 실패했거나 검증 중 하나라도 실패 — `main`에는 반영하지 않음 |
| `승인대기` | DB 변경 등 자동으로 처리하면 안 되는 작업 — 비고에 필요한 내용/목적/영향/다음 단계를 기록 |

### 13-3. 처음 설정하기 (완료됨)

1. Google Sheet 생성, 헤더 입력, 서비스 계정에 편집자 권한으로 공유 — **완료**
2. Google Cloud 서비스 계정 생성 + Sheets API 활성화 + JSON 키 발급 — **완료**
3. 로컬 `.env.local`에 아래 값 추가 (GitHub Secrets 불필요 — 전부 로컬에서만 씀):

```bash
GOOGLE_SHEET_ID=스프레드시트-ID
GOOGLE_SHEET_NAME=worklist
GOOGLE_SERVICE_ACCOUNT_KEY_FILE=./secrets/google-service-account.json
```

다운로드한 서비스 계정 JSON 키 파일은 프로젝트 루트의 `secrets/google-service-account.json`으로 저장하세요 (`secrets/`는 `.gitignore`에 등록되어 있어 커밋되지 않습니다).

### 13-4. 로컬에서 직접 확인하기

```bash
npm run worklist:list          # 처리 대상 행만 보기
npm run worklist:list -- --all # 전체 행 보기
npm run worklist:mark 5 완료 "설명 텍스트"
```

### 13-5. 나중에 무인 자동화가 필요해지면

`scripts/worklist/sheets.ts`(시트 읽기/쓰기), `eligibility.ts`(대상 행 선별), `riskyDiff.ts`(DB 변경 diff 안전장치)는 그대로 재사용할 수 있게 만들어져 있습니다. Claude Pro/Max 구독이 있다면 `claude setup-token`으로 발급한 구독 기반 토큰을 GitHub Actions Secret으로 등록해 무인 실행하는 방법도 있습니다(API 종량제 과금 없음) — 필요해지면 그때 워크플로만 추가하면 됩니다.
